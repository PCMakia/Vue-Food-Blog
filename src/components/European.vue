...
export default {
  ...
  async created() {
    try {
      const q = query(collection(db, "foods"), where("series", "==", "european"));
      ...
      console.error("Error fetching European recipes:", error);
    } finally {
      this.loading = false;
    }
  },
